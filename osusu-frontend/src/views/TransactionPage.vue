  ...
  <div class="w-full h-screen flex bg-gray-100">
    ...
    <div class="w-[100%] md:w-full min-h-[100%] flex flex-col items-center overflow-auto">
      ...
      <div class="w-full flex flex-col font-[500] px-[3%] text-[24px] mb-[40px]">
        ...
        <!-- filter section -->
        <div
          class="grid grid-cols-2 lg:flex flex-wrap justify-center items-center gap-4 bg-white p-4 rounded-lg shadow-md my-6 text-[16px]">
          <!-- Months Dropdown -->
            ...
            <table class="min-w-full ">
              ...
              <tbody>
                <tr v-for="(txn, index) in filteredTransactions" :key="index"
                  class="border-t text-[15px] lg:text-[16px] border-gray-200">
                  <td class="px-4 py-2">{{ txn.date }}</td>
                  <td class="px-4 py-2 font-semibold rounded" :class="txn.type === 'Deposit' ? 'bg-green-100 text-green-700'
                    : txn.type === 'Withdrawal' ? 'bg-red-100 text-red-700'
                      : 'bg-green-100 text-green-700'">
                    {{ txn.type }}
                  </td>
                  <td :class="txn.type === 'Withdrawal' ? 'text-red-600' : 'text-green-600'"
                    class="px-4 py-2 font-semibold">
                    {{ txn.amount }}
                  </td>
                  <td class="px-4 py-2">{{ txn.balance }}</td>
                  <td class="px-4 py-2 flex items-center gap-2">
                    <div class="min-w-[30px] h-[30px] text-[12px] flex items-center justify-center rounded-full"
                      :class="txn.type === 'Withdrawal' ? 'text-[red] bg-red-300' : 'text-[green] bg-green-300'">
                      <i :class="txn.type === 'Withdrawal' ? 'fa fa-arrow-up text-[red] bg-red-300'
                        : 'fa fa-arrow-down text-[green] bg-green-300'"></i>
                    </div>
                    {{ txn.description }}
                  </td>
                </tr>
